...
public class LobbyServer {
  ...
  // send match request to a User (String)
  public void sendMatchRequest(User userSender, String user) {
    boolean find = false;
    for (User client : this.clients) {
      if (client.getNickname().equals(user) && client != userSender) {
        find = true;
        client.getOutStream().println(
            "Match request: " + userSender.toString());
      }
    }
    if (!find) {
      userSender.getOutStream().println(userSender.toString() + " -> (<b>no one!</b>): " + "Match request");
    }
  }
  
  public void acceptMatchRequest(String msg, User userSender, String user) {
		boolean find = false;
	    for (User client : this.clients) {
	      if (client.getNickname().equals(user) && client != userSender) {
	        find = true;
	    ...
	    if (!find) {
	      userSender.getOutStream().println(userSender.toString() + " -> (<b>no one!</b>): " + "Match accept");
	    }
	}
}
...
class UserHandler implements Runnable {
  ...
  public UserHandler(LobbyServer server, User user) {
    this.server = server;
    this.user = user;
    ...
    while (sc.hasNextLine()) {
      ...
      if (message.charAt(0) == '@') {
        ...
        this.server.broadcastAllUsers();
	      ...
	      System.out.println("match request: " + message);
	      String userPrivate = message.substring(1, message.length());
	      server.sendMatchRequest(
	          user, userPrivate);
      } else if (message.charAt(0) == '&') {
    	  int firstSpace = message.indexOf(" ");
	      String userPrivate = message.substring(1, firstSpace);
	      int ipIndex = message.indexOf("IP:") + 3; // "+3" to move past "IP:"
	      String ip = message.substring(ipIndex).trim(); // trim to remove any leading or trailing spaces
        System.out.println("User private: " + userPrivate);
	      System.out.println("match accept: " + message);
	      System.out.println("IP: " + ip);
	      server.acceptMatchRequest(ip,
	          user, userPrivate);
      }
      else {
        // update user list
